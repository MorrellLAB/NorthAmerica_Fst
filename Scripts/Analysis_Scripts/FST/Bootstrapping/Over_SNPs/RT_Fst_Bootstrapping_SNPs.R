#Title: RT_Fst_Bootstrapping_SNPs.R
...
Samples_info_all<-read.table("~/Dropbox/mohsen_fst/Data/T3_download_04272015/ANAnames_samples_information.txt",header=T)
...
for (b in 1:100){
	...
	#look at the Fst at each locus
		Fst<-NULL
	     ...
	     Fst_all_RT<- cbind(as.data.frame(SNPs),Fst)
		
	write.table(Fst_all_RT,paste("Fst_RT_",b,".txt",sep=""),quote=F,row.names=F,col.names=F,sep="\t")
}